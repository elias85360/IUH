...
- `vite.config.js` proxies:
  - `/api`, `/metrics` -> `VITE_API_PROXY_TARGET` (default `http://localhost:4001`; set `http://localhost:8081` to exercise nginx)
  - `/kienlab` -> `VITE_MASTER_PROXY_TARGET` (set `http://localhost:8081` if you want it to flow through nginx)
